...
use sc_extract::{
    AssetConfig, AssetData, AssetSource, Datacore, DatacoreConfig, Guid,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    args.retain(|a| {
        if a == "--by-debug" { by_debug = true; false } else { true }
    });
    let needle = args.into_iter().next().ok_or("usage: contract_props_dig [--by-debug] <substring>")?.to_lowercase();
    ...
    for c in &index.contracts {
        let hay = if by_debug { c.debug_name.to_lowercase() } else { c.title(locale).unwrap_or("").to_lowercase() };
        if hay.contains(&needle) { matches.push(c); }
    }
    if matches.is_empty() { eprintln!("no match"); return Ok(()); }

    for c in &matches {
        let Some(ctx) = by_mission.get(&c.id) else { continue };
        ...
        if let Some(c) = ctx.parent_contract_handle.as_ref().and_then(|h| h.get(pools)) {
            println!("  [Contract] template:               {:?}", c.template);
            println!("  [Contract] additional_prerequisites: {} items", c.additional_prerequisites.len());
            println!("  [Contract] sub_contracts:           {} items", c.sub_contracts.len());
        ...
        if let Some(c) = ctx.parent_legacy_handle.as_ref().and_then(|h| h.get(pools)) {
            ...
            println!("  [ContractLegacy] additional_prerequisites: {} items", c.additional_prerequisites.len());
            println!("  [ContractLegacy] sub_contracts:     {} items", c.sub_contracts.len());
        ...
        if let Some(c) = ctx.parent_career_handle.as_ref().and_then(|h| h.get(pools)) {
            ...
            println!("  [CareerContract] additional_prerequisites: {} items", c.additional_prerequisites.len());
            println!("  [CareerContract] sub_contracts:     {} items", c.sub_contracts.len());
            template_guid = c.template.or(template_guid);
        }
        ...
        if let Some(s) = sub_contract {
            ...
            println!("  subContract.additional_prerequisites: {} items", s.additional_prerequisites.len());
            println!("  subContract.property_overrides:   {} items", s.property_overrides.len());
        }
        if let Some(t) = template_guid { signal_guids.insert(t); sources.push(("contract.template".into(), t)); }
        ...
        println!("\n── (3) ContractGenerator record ({}) ──", c.origin.generator_id);
        if let Some(rec) = db.record(&c.origin.generator_id) {
            let inst = rec.as_instance();
            for prop in inst.properties() {
                println!("  {}: {}", prop.name, describe_value(&prop.value));
                collect_guids_from_value(&prop.value, db, &mut signal_guids, &mut sources, &format!("generator.{}", prop.name));
            }
        } else {
            println!("  <not found>");
        }
        ...
        if let Some(ap) = ctx.handler_params.as_ref().and_then(|h| h.get(pools)) {
            ...
            println!("  bool_param_overrides: {} items", ap.bool_param_overrides.len());
            println!("  int_param_overrides:  {} items", ap.int_param_overrides.len());
            println!("  property_overrides:   {} items", ap.property_overrides.len());
        }
        if let Some(av) = ctx.handler_availability.as_ref().and_then(|h| h.get(pools)) {
            ...
            println!("  available_in_prison:             {}", av.available_in_prison);
            println!("  hide_in_mobi_glas:               {}", av.hide_in_mobi_glas);
            println!("  has_personal_cooldown:           {}", av.has_personal_cooldown);
            println!("  personal_cooldown_time:          {}", av.personal_cooldown_time);
            println!("  abandoned_cooldown_time:         {}", av.abandoned_cooldown_time);
            println!("  prerequisites:                   {} items", av.prerequisites.len());
        }
        ...
        println!("\n── (5) signal summary ──");
        println!("  total contract-side GUIDs collected: {}", signal_guids.len());
        println!("  of which resolve to tag names:        {}", name_signals.len());
                ...
                for (gi, gh) in val.spawn_descriptions.iter().enumerate() {
                    ...
                    for (si, sh) in group.ships.iter().enumerate() {
                        ...
                        println!(
                            ...
                            prop.mission_variable_name, so.options.len()
                        );
                        for (oi, oh) in so.options.iter().enumerate() {
                            let Some(opt) = oh.get(pools) else { continue };
                            let tag_guids: HashSet<Guid> = opt.tags.as_ref().and_then(|h| h.get(pools))
                                .map(|tl| tl.tags.iter().copied().collect())
                                .unwrap_or_default();
                            let tag_names: Vec<String> = tag_guids.iter()
                                .map(|g| tree.get(g).map(|n| n.name.clone()).unwrap_or_else(|| "?".into()))
                                .collect();
                            let guid_hits: Vec<Guid> = tag_guids.iter().filter(|g| signal_guids.contains(g)).copied().collect();
                            let name_hits: Vec<&String> = tag_names.iter().filter(|n| name_signals.contains(n.as_str())).collect();
                            println!(
                                "    opt[{oi}] c={} w={:.2} guid_hits={} name_hits={}  tags=[{}]",
                                opt.concurrent_amount, opt.weight,
                                guid_hits.len(), name_hits.len(),
                                tag_names.join(",")
                            );
                            for h in guid_hits {
                                let src = sources.iter().find(|(_, g)| *g == h).map(|(s, _)| s.as_str()).unwrap_or("?");
                                let tn = tree.get(&h).map(|n| n.name.as_str()).unwrap_or("?");
                                println!("      ✓ GUID hit: {tn}  (from {src})");
                            }
                            for n in name_hits {
                                println!("      ~ name hit: {n}  (no GUID match — name happens to equal a signal tag's name)");
                            }
                        }
                    }
                }
...
fn describe_value(v: &sc_contracts::raw::Value<'_>) -> String {
    ...
    match v {
        ...
        V::ClassRef(r) => format!("ClassRef[struct={},instance={}]", r.struct_index, r.instance_index),
        V::StrongPointer(Some(r)) => format!("StrongPtr[struct={},instance={}]", r.struct_index, r.instance_index),
        V::WeakPointer(Some(r)) => format!("WeakPtr[struct={},instance={}]", r.struct_index, r.instance_index),
    ...
    for prop in inst.properties() {
        ...
        match &prop.value {
            V::Class { struct_index, data } => {
                ...
                dump_instance_deep(&nested, &sub_label, depth + 1, max_depth, db, signal_guids, sources);
            }
            V::ClassRef(r) => {
            ...
            V::StrongPointer(Some(r)) | V::WeakPointer(Some(r)) => {
                let nested = db.instance(r.struct_index, r.instance_index);
                let sname = db.struct_name(r.struct_index.try_into().unwrap_or(0)).unwrap_or("?");
                ...
                if let Some(arr) = inst.get_array(prop.name) {
                    for (i, av) in arr.enumerate() {
                        match av {
                            V::Class { struct_index, data } => {
                                let nested = sc_contracts::raw::Instance::from_inline_data(db, struct_index, data);
                                dump_instance_deep(&nested, &format!("{sub_label}[{i}]"), depth + 1, max_depth, db, signal_guids, sources);
                            }
                            V::ClassRef(r) => {
                                let nested = db.instance(r.struct_index, r.instance_index);
                                dump_instance_deep(&nested, &format!("{sub_label}[{i}]"), depth + 1, max_depth, db, signal_guids, sources);
...
fn collect_guids_from_value(
    ...
    use sc_contracts::raw::Value as V;
    match v {
        V::Guid(g) => { out.insert(*g); sources.push((label.into(), *g)); }
        V::Reference(Some(r)) => { out.insert(r.guid); sources.push((label.into(), r.guid)); }
        _ => {}
    }
}
    ...
    for (_g, gh) in &datacore.records().records.multi_feature.contract_generator {
        ...
        for handler_ptr in &generator.generators {
            match handler_ptr {
                ...
                H::ContractGeneratorHandler_List(h) => take_contracts(h.get(pools).map(|h| (h.contract_params.clone(), h.default_availability.clone(), h.contracts.clone())), pools, out),
                H::ContractGeneratorHandler_LinearSeries(h) => take_contracts(h.get(pools).map(|h| (h.contract_params.clone(), h.default_availability.clone(), h.contracts.clone())), pools, out),
                H::ContractGeneratorHandler_TutorialSeriesDef(h) => take_contracts(h.get(pools).map(|h| (h.contract_params.clone(), h.default_availability.clone(), h.contracts.clone())), pools, out),
                _ => {}
            }
        }
    }
                ...
                && let Some(handler) = h.get(pools)
            {
                ...
                for ch in &handler.legacy_contracts {
                    ...
                    out.entry(c.id).or_insert(MissionContext {
                        handler_params: hp.clone(), handler_availability: ha.clone(),
...
fn take_contracts(
    handler: Option<(Option<Handle<ContractParamOverrides>>, Option<Handle<sc_extract::generated::ContractAvailability>>, Vec<Handle<sc_extract::generated::Contract>>)>,
    pools: &DataPools,
    out: &mut HashMap<Guid, MissionContext>,
) {
    let Some((hp, ha, contracts)) = handler else { return };
    for ch in &contracts {
        let Some(c) = ch.get(pools) else { continue };
        out.entry(c.id).or_insert(MissionContext {
            handler_params: hp.clone(), handler_availability: ha.clone(),
