...
impl Pooled for RadarPlate_Config {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.ui_radardisplay_config_radar.radar_plate_config
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.ui_radardisplay_config_radar.radar_plate_config
    }
}
        ...
        Self {
            ...
            main_plate_sprite: match inst.get("MainPlateSprite") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<SimpleSpriteSlot>(
            ...
            bottom_aring_size_multiplier: inst
                .get_f32("BottomARingSizeMultiplier")
                .unwrap_or_default(),
            ...
            bottom_bring_size_multiplier: inst
                .get_f32("BottomBRingSizeMultiplier")
                .unwrap_or_default(),
...
impl Pooled for RadarDisplayEntryEffects_Config {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            ...
            .radar_display_entry_effects_config
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .ui_radardisplay_config_radar
...
impl<'a> Extract<'a> for RadarDisplayEntryEffects_Config {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            ...
            activate_ping_relative_color_change: inst
                .get_f32("activatePingRelativeColorChange")
                .unwrap_or_default(),
...
impl Pooled for RadarDisplay_Config {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.ui_radardisplay_config_radar.radar_display_config
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.ui_radardisplay_config_radar.radar_display_config
    }
}
...
impl<'a> Extract<'a> for RadarDisplay_Config {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            active_ping_color: HUDPalleteEntry::from_dcb_str(
                inst.get_str("ActivePingColor").unwrap_or(""),
            ),
            radar_entry_effects: match inst.get("radarEntryEffects") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<RadarDisplayEntryEffects_Config>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            radar_plate_settings: match inst.get("radarPlateSettings") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<RadarPlate_Config>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
