...
impl Pooled for MissionFailConditionParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.missionfailureconditions.mission_fail_condition_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.missionfailureconditions.mission_fail_condition_params
    }
}
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            ...
            display_text: inst
                ...
                .map(LocaleKey::from)
                .unwrap_or_default(),
            use_automatic_failure_screen: inst
                .get_bool("useAutomaticFailureScreen")
                .unwrap_or_default(),
        }
    }
}
...
impl Pooled for MissionFailConditionsList {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.missionfailureconditions.mission_fail_conditions_list
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.missionfailureconditions.mission_fail_conditions_list
    }
}
...
impl<'a> Extract<'a> for MissionFailConditionsList {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            failure_conditions: inst
                .get_array("failureConditions")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<MissionFailConditionParams>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<MissionFailConditionParams>(
