...
impl Pooled for SecurityNetworkRoomSettings {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.lawsystem.security_network_room_settings
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.lawsystem.security_network_room_settings
    }
}
        ...
        Self {
            default_protocol: match inst.get("defaultProtocol") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<SecurityNetworkProtocol>(
            ...
            room_identifier: inst
                .get("roomIdentifier")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            trespass_warning_seconds: inst.get_f32("trespassWarningSeconds").unwrap_or_default(),
            trespass_revoke_warning_seconds: inst
                .get_f32("trespassRevokeWarningSeconds")
                .unwrap_or_default(),
        }
    }
}
