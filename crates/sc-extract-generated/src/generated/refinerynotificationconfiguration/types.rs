    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            text: inst
                ...
                .unwrap_or_default(),
            text_multiple: inst
                .get_str("textMultiple")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            duration: inst.get_f32("duration").unwrap_or_default(),
            refinery_service_error: inst
                .get_str("refineryServiceError")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            refinery_delivery_failed: inst
                .get_str("refineryDeliveryFailed")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            refinery_job_creation_failed: inst
                .get_str("refineryJobCreationFailed")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            refinery_setup_error: inst
                .get_str("refinerySetupError")
                .map(LocaleKey::from)
                .unwrap_or_default(),
        }
    }
