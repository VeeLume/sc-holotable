        ...
        Self {
            variables: inst.get_array("variables")
                .map(|arr| arr.filter_map(|v| match v {
                        Value::StrongPointer(Some(r)) | Value::WeakPointer(Some(r)) => Some(MissionVariableBasePtr::from_ref(b, r)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
...
impl Pooled for MissionVariableBoolean {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_variable_boolean }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_variable_boolean }
}
...
impl Pooled for MissionVariableInteger {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_variable_integer }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_variable_integer }
}
...
impl Pooled for LocationEntityType_StaticEntity {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.location_entity_type_static_entity }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.location_entity_type_static_entity }
}
...
impl Pooled for ModuleLocationEntities {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.module_location_entities }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.module_location_entities }
}
...
impl Pooled for ModuleLocationEntities_Static {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.module_location_entities_static }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.module_location_entities_static }
}
...
impl<'a> Extract<'a> for ModuleLocationEntities_Static {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            debug_name: inst.get_str("debugName").map(String::from).unwrap_or_default(),
            optional: inst.get_bool("optional").unwrap_or_default(),
            entity_declarations: inst.get_array("entityDeclarations")
...
impl Pooled for ModuleDeclarationType_Mission {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.module_declaration_type_mission }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.module_declaration_type_mission }
}
...
impl<'a> Extract<'a> for ModuleDeclarationType_Mission {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            location_entities: inst.get_array("locationEntities")
                .map(|arr| arr.filter_map(|v| match v {
                        Value::StrongPointer(Some(r)) | Value::WeakPointer(Some(r)) => Some(ModuleLocationEntitiesPtr::from_ref(b, r)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
            required_location_modules: inst.get_array("requiredLocationModules")
                .map(|arr| arr.filter_map(|v| if let Value::Reference(Some(r)) = v { Some(r.guid) } else { None }).collect())
                .unwrap_or_default(),
            required_resource_slot: inst.get("requiredResourceSlot").and_then(|v| v.as_record_ref()).map(|r| r.guid),
        }
    }
}
...
impl Pooled for ModuleDeclarationType_Location {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.module_declaration_type_location }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.module_declaration_type_location }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            module_type: match inst.get("moduleType") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => Some(ModuleDeclarationType_BasePtr::from_ref(b, r)),
                _ => None,
            },
            properties: inst.get_array("properties")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<MissionProperty>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
...
impl Pooled for MissionInitParamBoolean {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_init_param_boolean }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_init_param_boolean }
}
...
impl Pooled for MissionInitParamInteger {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_init_param_integer }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_init_param_integer }
}
...
impl Pooled for MissionInitParamTag {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_init_param_tag }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_init_param_tag }
}
...
impl Pooled for MissionInitParamFloat {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.missiondata.mission_init_param_float }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.missiondata.mission_init_param_float }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            spawn_group: match inst.get("spawnGroup") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => Some(BaseMissionPropertyValuePtr::from_ref(b, r)),
                _ => None,
            },
        }
    }
        ...
        Self {
            spawn_descriptions: inst.get_array("spawnDescriptions")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<SpawnDescriptionEntry>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
