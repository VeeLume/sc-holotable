...
impl Pooled for InteractionConditionCanAffordItem {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            ...
            .interaction_condition_can_afford_item
    }
...
impl Pooled for InteractionConditionCanBeBodyDragged {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .interactionconditions
            .interaction_condition_can_be_body_dragged
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .interactionconditions
...
impl Pooled for ShopInteractionData {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.interactionconditions.shop_interaction_data
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.interactionconditions.shop_interaction_data
    }
}
        ...
        Self {
            quick_buy_condition_list: match inst.get("quickBuyConditionList") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<InteractionConditionList>(
            ...
            quick_buy_interaction_text: inst
                .get_str("quickBuyInteractionText")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            quick_buy_price_string_token: inst
                .get_str("quickBuyPriceStringToken")
                .map(String::from)
                .unwrap_or_default(),
            more_info_interaction_text: inst
                .get_str("moreInfoInteractionText")
                .map(LocaleKey::from)
                .unwrap_or_default(),
        }
    }
