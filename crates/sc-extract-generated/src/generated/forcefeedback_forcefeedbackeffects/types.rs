...
impl Pooled for ForceFeedback {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.forcefeedback_forcefeedbackeffects.force_feedback
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.forcefeedback_forcefeedbackeffects.force_feedback
    }
}
        ...
        Self {
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ForceFeedbackPattern>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ForceFeedbackPattern>(
            ...
            envelopes: inst
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ForceFeedbackEnvelope>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ForceFeedbackEnvelope>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
            ...
            effects: inst
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ForceFeedbackEffect>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ForceFeedbackEffect>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
        ...
        Self {
            ...
            motor_ab: match inst.get("MotorAB") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<ForceFeedbackMotor>(
                        b.db.instance(r.struct_index, r.instance_index),
                        true,
                    ))
                }
