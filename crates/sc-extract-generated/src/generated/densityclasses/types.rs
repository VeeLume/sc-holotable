...
impl Pooled for SDensityClassLifetimeOverrideEntry {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.densityclasses.sdensity_class_lifetime_override_entry
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.densityclasses.sdensity_class_lifetime_override_entry
    }
}
...
impl Pooled for SDefaultDensityClassLifetimeOverrides {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .densityclasses
            .sdefault_density_class_lifetime_overrides
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .densityclasses
...
impl Pooled for SEntityDensityClassOverridesRecord {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.densityclasses.sentity_density_class_overrides_record
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.densityclasses.sentity_density_class_overrides_record
    }
}
        ...
        Self {
            overrides: match inst.get("overrides") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<SEntityDensityClassOverridesManual>(
...
impl Pooled for SEntityDensityClassOverridesManual {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.densityclasses.sentity_density_class_overrides_manual
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.densityclasses.sentity_density_class_overrides_manual
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            defaults: match inst.get("defaults") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<SDefaultDensityClassLifetimeOverrides>(
            ...
            density_class_lifetime_overrides: inst
                .get_array("densityClassLifetimeOverrides")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<SDensityClassLifetimeOverrideEntry>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
