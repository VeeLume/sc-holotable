...
//! Types for feature `entities-shopkiosk`.
...
impl Pooled for PurchasableVehicleProviderParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.entities_shopkiosk.purchasable_vehicle_provider_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.entities_shopkiosk.purchasable_vehicle_provider_params
    }
}
...
impl<'a> Extract<'a> for PurchasableVehicleProviderParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            vehicle_usage_types: PurchasableVehicleUsageType::from_dcb_str(
                inst.get_str("vehicleUsageTypes").unwrap_or(""),
            ),
        }
    }
}
