...
pub struct IfcsInputDeflectionTimeParams {
    /// DCB field: `linearMovement` (Class)
    #[serde(default)]
...
impl Pooled for IfcsInputDeflectionTimeParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.b_if.ifcs_input_deflection_time_params }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.b_if.ifcs_input_deflection_time_params }
}
