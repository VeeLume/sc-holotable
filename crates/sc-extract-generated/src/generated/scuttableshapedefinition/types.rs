...
impl Pooled for SCuttableShapeDefinition {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.scuttableshapedefinition.scuttable_shape_definition
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.scuttableshapedefinition.scuttable_shape_definition
    }
}
...
impl<'a> Extract<'a> for SCuttableShapeDefinition {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            shape_name_prefix: inst
                .get_str("shapeNamePrefix")
                .map(String::from)
                .unwrap_or_default(),
        }
    }
