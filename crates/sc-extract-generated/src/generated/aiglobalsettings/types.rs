...
impl Pooled for AIFormulaScoreModifiers {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.aiglobalsettings.aiformula_score_modifiers
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.aiglobalsettings.aiformula_score_modifiers
    }
}
...
impl Pooled for AITargetingFormulaSettings {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.aiglobalsettings.aitargeting_formula_settings
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.aiglobalsettings.aitargeting_formula_settings
    }
}
...
impl<'a> Extract<'a> for AITargetingFormulaSettings {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            in_vehicle_range_multiplier: inst
                .get_f32("inVehicleRangeMultiplier")
                .unwrap_or_default(),
            self_defence_max_health_multiplier: inst
                .get_f32("selfDefenceMaxHealthMultiplier")
                .unwrap_or_default(),
            protected_max_health_multiplier: inst
                .get_f32("protectedMaxHealthMultiplier")
                .unwrap_or_default(),
            recent_damage_decay_factor_per_second: inst
                .get_f32("recentDamageDecayFactorPerSecond")
                .unwrap_or_default(),
            attacker_capacity_score: match inst.get("attackerCapacityScore") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<AIFormulaScoreModifiers>(
