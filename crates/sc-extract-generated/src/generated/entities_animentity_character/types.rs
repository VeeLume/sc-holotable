...
impl Pooled for SItemPortActorRecordParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .entities_animentity_character
            .sitem_port_actor_record_params
    }
        ...
        Self {
            wear_range: match inst.get("WearRange") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => Some(
                    b.alloc_nested::<Range>(b.db.instance(r.struct_index, r.instance_index), true),
                ),
                _ => None,
            },
            ...
            skip_inventory_items_on_mission_entities: inst
                .get_bool("SkipInventoryItemsOnMissionEntities")
                .unwrap_or_default(),
            inventory_items: inst
                .get_array("InventoryItems")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<SLoadoutInventoryItem>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<SLoadoutInventoryItem>(
            ...
            actor_record: inst
                ...
                .map(|r| r.guid),
            outfit_name_tags: inst
                .get_array("OutfitNameTags")
                .map(|arr| {
                    arr.filter_map(|v| {
