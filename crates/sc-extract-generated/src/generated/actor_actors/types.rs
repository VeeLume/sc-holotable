...
impl Pooled for SDynamicGroupComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.sdynamic_group_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.sdynamic_group_component_params
    }
}
...
impl Pooled for ActivityBehaviorComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.activity_behavior_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.activity_behavior_component_params
    }
}
...
impl Pooled for TerrainTrashCleanupComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.terrain_trash_cleanup_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.terrain_trash_cleanup_component_params
    }
}
...
impl<'a> Extract<'a> for TerrainTrashCleanupComponentParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            ...
            decay_bounding_box_multiplier: inst
                .get_f32("decayBoundingBoxMultiplier")
                .unwrap_or_default(),
...
impl Pooled for SActorDeathPoseVariant {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.sactor_death_pose_variant
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.sactor_death_pose_variant
    }
}
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            variant_name: inst
                ...
                .unwrap_or_default(),
            mannequin_tag: inst
                .get_str("mannequinTag")
                .map(String::from)
                .unwrap_or_default(),
            geometry_tags: inst
                .get_str("geometryTags")
                .map(String::from)
                .unwrap_or_default(),
        }
    }
...
impl Pooled for SActorStaticColliderDeathBehaviour {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.sactor_static_collider_death_behaviour
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.sactor_static_collider_death_behaviour
    }
}
...
impl<'a> Extract<'a> for SActorStaticColliderDeathBehaviour {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            density_class_override: inst
                .get("densityClassOverride")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            dead_collider_class: inst
                .get("deadColliderClass")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            death_pose_variants: inst
                .get_array("deathPoseVariants")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<SActorDeathPoseVariant>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<SActorDeathPoseVariant>(
                ...
                .unwrap_or_default(),
            death_orientation_entity_tag: inst
                .get("deathOrientationEntityTag")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
...
impl Pooled for SDummyPlayerComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.sdummy_player_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.sdummy_player_component_params
    }
}
...
impl<'a> Extract<'a> for SDummyPlayerComponentParams {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            max_delta_angle_rate_normal: inst
                .get_f32("maxDeltaAngleRateNormal")
                .unwrap_or_default(),
            jump_fall_land_params: inst
                .get("jumpFallLandParams")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            aim_iklayer: inst.get_i32("aimIKLayer").unwrap_or_default(),
            look_iklayer: inst.get_i32("lookIKLayer").unwrap_or_default(),
            character_name_data: inst
                .get("characterNameData")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            skeleton_config: inst
                .get("skeletonConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            ledge_grabbing_params: inst
                .get("ledgeGrabbingParams")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            jump_fall_land_config: inst
                .get("jumpFallLandConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            carry_config: inst
                .get("carryConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            player_animated_interaction_config: inst
                .get("playerAnimatedInteractionConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            ladder_config: inst
                .get("ladderConfig")
            ...
            movement_modifiers_record: inst
                .get("movementModifiersRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            movement_sets_record: inst
                .get("movementSetsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            is_minimal: inst.get_bool("isMinimal").unwrap_or_default(),
            ai_default_stance: ActorStateFilterByStanceState::from_dcb_str(
                inst.get_str("aiDefaultStance").unwrap_or(""),
            ),
            actor_type: EActorType::from_dcb_str(inst.get_str("actorType").unwrap_or("")),
            stances_data_record: inst
                .get("stancesDataRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_status_record: inst
                .get("actorStatusRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_status_record_ea: inst
                .get("actorStatusRecordEA")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_status_record_sq42: inst
                .get("actorStatusRecordSQ42")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            take_down_config: inst
                .get("takeDownConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            misfire_fix_config: inst
                .get("misfireFixConfig")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            hit_reactions_def_record: inst
                .get("hitReactionsDefRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            force_reactions_def_record: inst
                .get("forceReactionsDefRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            shake_record_first_person: inst
                .get("shakeRecordFirstPerson")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            shake_record_third_person: inst
                .get("shakeRecordThirdPerson")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_aim_limits: inst
                .get("actorAimLimits")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_look_limits: inst
                .get("actorLookLimits")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            turn_angles: match inst.get("turnAngles") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<ActorTurnAngles>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            ducking_params: inst
                ...
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            gforce_params_record: inst
                .get("gforceParamsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            gforce_params_record_master_modes: inst
                .get("gforceParamsRecordMasterModes")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            gforce_head_bob_record: inst
                .get("gforceHeadBobRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            gforce_camera_effects_record: inst
                .get("gforceCameraEffectsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            ads_camera_default_params: match inst.get("adsCameraDefaultParams") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<AdsCameraParams>(
                        b.db.instance(r.struct_index, r.instance_index),
                        true,
                    ))
                }
            ...
            ability_params_record: inst
                .get("abilityParamsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            breathing_params_record: inst
                .get("breathingParamsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            stamina_params_record: inst
                .get("staminaParamsRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_environment_record: inst
                .get("actorEnvironmentRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            default_look_ahead_record: inst
                .get("defaultLookAheadRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            locomotion_personality_record: inst
                .get("locomotionPersonalityRecord")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            weapon_ports_params: match inst.get("weaponPortsParams") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<WeaponPortsParams>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            wingman_orders: inst
                ...
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_targeted_params: inst
                .get("actorTargetedParams")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_sliding_params: inst
                .get("actorSlidingParams")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            actor_zero_gtraversal_params: inst
                .get("actorZeroGTraversalParams")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            base_foley_def: inst
                .get("baseFoleyDef")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            base_footstep_def: inst
                .get("baseFootstepDef")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            death_behaviour: match inst.get("deathBehaviour") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(SActorDeathBehaviourPtr::from_ref(b, r))
                }
                _ => None,
            },
        }
...
impl Pooled for TerrainTrashCleanupGameplayTrigger {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.terrain_trash_cleanup_gameplay_trigger
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.terrain_trash_cleanup_gameplay_trigger
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            overrides: match inst.get("overrides") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<TerrainTrashCleanupComponentParams>(
                        b.db.instance(r.struct_index, r.instance_index),
                        true,
                    ))
                }
...
impl Pooled for UndergroundCreatureHelpersComponent {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.actor_actors.underground_creature_helpers_component
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.actor_actors.underground_creature_helpers_component
    }
}
...
impl<'a> Extract<'a> for UndergroundCreatureHelpersComponent {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            spine_joints: inst
                .get_array("spineJoints")
                .map(|arr| arr.filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default(),
            tail_joints: inst
                .get_array("tailJoints")
                .map(|arr| arr.filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default(),
            surface_vfxjoint: inst
                .get_str("surfaceVFXJoint")
                .map(String::from)
                .unwrap_or_default(),
            ground_exit_vfxjoint: inst
                .get_str("groundExitVFXJoint")
                .map(String::from)
                .unwrap_or_default(),
            ground_entry_vfxjoint: inst
                .get_str("groundEntryVFXJoint")
                .map(String::from)
                .unwrap_or_default(),
            spawn_molehill_entity: inst.get_bool("spawnMolehillEntity").unwrap_or_default(),
            num_of_starting_molehill_entities: inst
                .get_u32("numOfStartingMolehillEntities")
                .unwrap_or_default(),
            start_exit_decaying_immediately: inst
                .get_bool("startExitDecayingImmediately")
                .unwrap_or_default(),
            override_exit_decay_time_to_start: inst
                .get_f32("overrideExitDecayTimeToStart")
                .unwrap_or_default(),
            molehill_class: inst
                .get("molehillClass")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            trigger_emerge_molehill_at_animation_start: inst
                .get_bool("triggerEmergeMolehillAtAnimationStart")
                .unwrap_or_default(),
        }
    }
}
