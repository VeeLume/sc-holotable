...
impl Pooled for ArenaCommanderLocationObjectContainersParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .megamap
            .arena_commander_location_object_containers_params
    }
...
impl Pooled for ArenaCommanderPlanetOverrideParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.megamap.arena_commander_planet_override_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.megamap.arena_commander_planet_override_params
    }
}
...
pub struct ArenaCommanderScenarioParams {
    ...
    pub location_object_containers_params:
        Option<Handle<ArenaCommanderLocationObjectContainersParams>>,
...
impl Pooled for ArenaCommanderScenarioParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.megamap.arena_commander_scenario_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.megamap.arena_commander_scenario_params
    }
}
        ...
        Self {
            location_object_containers_params: match inst.get("locationObjectContainersParams") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => Some(
                    b.alloc_nested::<ArenaCommanderLocationObjectContainersParams>(
            ...
            planet_override_params: match inst.get("planetOverrideParams") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<ArenaCommanderPlanetOverrideParams>(
                        b.db.instance(r.struct_index, r.instance_index),
                        true,
                    ))
                }
...
impl Pooled for SMegaMapSolarSystem {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.megamap.smega_map_solar_system
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.megamap.smega_map_solar_system
    }
}
...
impl Pooled for MegaMap {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.megamap.mega_map
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.megamap.mega_map
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            game_mode: inst
                ...
                .map(|r| r.guid),
            solar_systems: inst
                .get_array("SolarSystems")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<SMegaMapSolarSystem>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<SMegaMapSolarSystem>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
                ...
                .unwrap_or_default(),
            single_player_or_multiplayer: SinglePlayerOrMultiplayer::from_dcb_str(
                inst.get_str("singlePlayerOrMultiplayer").unwrap_or(""),
            ),
            subsumption_mission: inst
                .get_str("subsumptionMission")
                .map(String::from)
                .unwrap_or_default(),
            subsumption_mission_init_params: inst
                .get_array("subsumptionMissionInitParams")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::StrongPointer(Some(r)) | Value::WeakPointer(Some(r)) => {
                            Some(AbstractMissionInitParamPtr::from_ref(b, r))
                        }
                        _ => None,
                    })
                    .collect()
            ...
            arena_commander_scenario_params: match inst.get("arenaCommanderScenarioParams") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(b.alloc_nested::<ArenaCommanderScenarioParams>(
                        b.db.instance(r.struct_index, r.instance_index),
                        true,
                    ))
                }
            ...
            level: inst
                ...
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            track_view_intro: inst
                .get_str("trackViewIntro")
                .map(String::from)
                .unwrap_or_default(),
            root_location: inst
                .get("rootLocation")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            streaming_mode: LevelStreamingMode::from_dcb_str(
                inst.get_str("streamingMode").unwrap_or(""),
            ),
            bind_culling_enabled: inst.get_bool("bindCullingEnabled").unwrap_or_default(),
            default_winning_team_override: inst
                .get_i32("defaultWinningTeamOverride")
                .unwrap_or_default(),
            display_name: inst
                .get_str("displayName")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            appears_in_s42_level_select: inst
                .get_bool("appearsInS42LevelSelect")
                .unwrap_or_default(),
            chapter: inst
                .get("chapter")
                .and_then(|v| v.as_record_ref())
