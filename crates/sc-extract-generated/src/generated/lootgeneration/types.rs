...
impl Pooled for LootGenerationSpecialEventArchetype {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.lootgeneration.loot_generation_special_event_archetype
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.lootgeneration.loot_generation_special_event_archetype
    }
}
        ...
        Self {
            ...
            archetype_v3: match inst.get("archetypeV3") {
                Some(Value::StrongPointer(Some(r))) | Some(Value::WeakPointer(Some(r))) => {
                    Some(LootArchetypeV3_BasePtr::from_ref(b, r))
                }
                _ => None,
            },
        }
...
impl Pooled for LootGenerationGlobalParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.lootgeneration.loot_generation_global_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.lootgeneration.loot_generation_global_params
    }
}
...
impl<'a> Extract<'a> for LootGenerationGlobalParams {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            special_event_archetypes: inst
                .get_array("specialEventArchetypes")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<LootGenerationSpecialEventArchetype>(
