...
impl Pooled for AIWaveCollection {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.aiwavecollection.aiwave_collection
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.aiwavecollection.aiwave_collection
    }
}
            ...
            waves: inst
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => Some(b.alloc_nested::<AIWave>(
                            Instance::from_inline_data(b.db, struct_index, data),
                            false,
                        )),
                        Value::ClassRef(r) => Some(b.alloc_nested::<AIWave>(
...
impl Pooled for AIWave {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.aiwavecollection.aiwave
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.aiwavecollection.aiwave
    }
}
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<AIWaveMember>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<AIWaveMember>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
...
impl Pooled for AIWaveMember {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.aiwavecollection.aiwave_member
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.aiwavecollection.aiwave_member
    }
}
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            archetype: inst
                ...
                .map(String::from)
                .unwrap_or_default(),
            entity_class_definition: inst
                .get("entityClassDefinition")
            ...
            crew_manifest_override: inst
                .get("crewManifestOverride")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            skillset_override: inst
                .get("skillsetOverride")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            cargo_manifests: inst
                .get_array("cargoManifests")
                .map(|arr| {
                    arr.filter_map(|v| {
                ...
                .unwrap_or_default(),
            ai_damage_modifiers_override: inst
                .get("aiDamageModifiersOverride")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
        }
    }
