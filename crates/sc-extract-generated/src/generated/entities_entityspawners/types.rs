...
impl Pooled for SSpawnOnlyOnRequest {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.entities_entityspawners.sspawn_only_on_request }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.entities_entityspawners.sspawn_only_on_request }
}
...
impl Pooled for SSequencerSpawnInUseableSpawnerTaskParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.entities_entityspawners.ssequencer_spawn_in_useable_spawner_task_params }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.entities_entityspawners.ssequencer_spawn_in_useable_spawner_task_params }
}
...
impl<'a> Extract<'a> for SSequencerSpawnInUseableSpawnerTaskParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            entity_to_spawn: inst.get("entityToSpawn").and_then(|v| v.as_record_ref()).map(|r| r.guid),
            use_channel: inst.get("useChannel").and_then(|v| v.as_record_ref()).map(|r| r.guid),
