...
impl Pooled for ChatEmoteRecord {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatemoterecord.chat_emote_record
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatemoterecord.chat_emote_record
    }
}
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ChatEmotePack>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ChatEmotePack>(
...
impl Pooled for ChatEmotePack {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatemoterecord.chat_emote_pack
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatemoterecord.chat_emote_pack
    }
}
        ...
        Self {
            ...
            emotes: inst
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ChatEmoteData>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ChatEmoteData>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
...
impl Pooled for ChatEmoteData {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatemoterecord.chat_emote_data
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatemoterecord.chat_emote_data
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            emote_type: inst
                ...
                .map(LocaleKey::from)
                .unwrap_or_default(),
            alternate_emote_types: inst
                .get_array("alternateEmoteTypes")
                .map(|arr| {
            ...
            anim_data: match inst.get("animData") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<ChatEmoteAnimData>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
...
impl Pooled for ChatEmoteAnimData {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatemoterecord.chat_emote_anim_data
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatemoterecord.chat_emote_anim_data
    }
}
...
impl<'a> Extract<'a> for ChatEmoteAnimData {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            ...
            text_to_display: inst
                .get_str("textToDisplay")
                .map(LocaleKey::from)
                .unwrap_or_default(),
