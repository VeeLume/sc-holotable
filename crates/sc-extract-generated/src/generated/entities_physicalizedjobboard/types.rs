...
impl Pooled for SMissionAcceptGameplayTriggerNotifications {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .entities_physicalizedjobboard
            .smission_accept_gameplay_trigger_notifications
    }
...
impl<'a> Extract<'a> for SMissionAcceptGameplayTriggerNotifications {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            unknown_error_notification: inst
                .get("unknownErrorNotification")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            no_matching_contracts_found_notification: inst
                .get("noMatchingContractsFoundNotification")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            mission_already_active_notification: inst
                .get("missionAlreadyActiveNotification")
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
        }
    }
...
impl Pooled for SMissionAcceptGameplayTrigger {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .entities_physicalizedjobboard
            .smission_accept_gameplay_trigger
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .entities_physicalizedjobboard
...
impl<'a> Extract<'a> for SMissionAcceptGameplayTrigger {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            debug_contract_name: inst
                .get_str("debugContractName")
                .map(String::from)
                .unwrap_or_default(),
            notifications: match inst.get("notifications") {
                Some(Value::Class { struct_index, data }) => Some(
                    b.alloc_nested::<SMissionAcceptGameplayTriggerNotifications>(
