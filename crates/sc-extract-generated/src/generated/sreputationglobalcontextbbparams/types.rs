...
impl<'a> Extract<'a> for SReputationContextBBEntityListParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            entity_tab_name: inst.get_str("entityTabName").map(LocaleKey::from).unwrap_or_default(),
            entity_type: EReputationEntityType::from_dcb_str(inst.get_str("entityType").unwrap_or("")),
        }
    }
}
        ...
        Self {
            ...
            entity_tabs: inst.get_array("entityTabs")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<SReputationContextBBEntityListParams>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
            entity_sort_order: EReputationSortOrderEntity::from_dcb_str(inst.get_str("entitySortOrder").unwrap_or("")),
        }
    }
}
