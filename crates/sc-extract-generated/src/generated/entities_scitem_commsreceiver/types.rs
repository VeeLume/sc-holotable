...
impl Pooled for SCItemCommsReceiverComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.entities_scitem_commsreceiver.scitem_comms_receiver_component_params }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.entities_scitem_commsreceiver.scitem_comms_receiver_component_params }
}
...
impl<'a> Extract<'a> for SCItemCommsReceiverComponentParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            setup: inst.get("setup").and_then(|v| v.as_record_ref()).map(|r| r.guid),
            comms_channels: inst.get_array("commsChannels")
                .map(|arr| arr.filter_map(|v| if let Value::Reference(Some(r)) = v { Some(r.guid) } else { None }).collect())
                .unwrap_or_default(),
            audio_comms_effect_override: inst.get_str("audioCommsEffectOverride").map(String::from).unwrap_or_default(),
        }
    }
}
