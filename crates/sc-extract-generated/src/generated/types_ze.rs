...
pub struct ZeroGTraversalConnection {
    ...
    /// DCB field: `resetViewOnTransition` (Boolean)
    #[serde(default)]
    pub reset_view_on_transition: bool,
    /// DCB field: `playExitAnimation` (Boolean)
    #[serde(default)]
    pub play_exit_animation: bool,
    /// DCB field: `allowExitYield` (Boolean)
    #[serde(default)]
...
impl Pooled for ZeroGTraversalConnection {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.b_ze.zero_gtraversal_connection }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.b_ze.zero_gtraversal_connection }
}
...
pub struct ZeroGTraversalState {
    ...
    /// DCB field: `playEnterAnimation` (Boolean)
    #[serde(default)]
    pub play_enter_animation: bool,
    /// DCB field: `canTurnInState` (Boolean)
    #[serde(default)]
    pub can_turn_in_state: bool,
    /// DCB field: `useAnimMotionControl` (Boolean)
    #[serde(default)]
...
impl Pooled for ZeroGTraversalState {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.b_ze.zero_gtraversal_state }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.b_ze.zero_gtraversal_state }
}
...
pub struct ZeroGTraversalGraph {
    /// DCB field: `traversalStates` (Class (array))
    #[serde(default)]
    pub traversal_states: Vec<Handle<ZeroGTraversalState>>,
}

impl Pooled for ZeroGTraversalGraph {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.b_ze.zero_gtraversal_graph }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.b_ze.zero_gtraversal_graph }
}
