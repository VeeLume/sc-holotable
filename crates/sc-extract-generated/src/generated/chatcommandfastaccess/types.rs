...
impl Pooled for ChatCommandFastAccess {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatcommandfastaccess.chat_command_fast_access
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatcommandfastaccess.chat_command_fast_access
    }
}
                ...
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<ChatCommandName>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<ChatCommandName>(
...
impl Pooled for ChatCommandName {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.chatcommandfastaccess.chat_command_name
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.chatcommandfastaccess.chat_command_name
    }
}
