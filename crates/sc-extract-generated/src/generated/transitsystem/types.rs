...
impl Pooled for TransitNavigationLink {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_navigation_link
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_navigation_link
    }
}
...
impl Pooled for HospitalEmergencyScreenComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .transitsystem
            .hospital_emergency_screen_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .transitsystem
...
impl<'a> Extract<'a> for HospitalEmergencyScreenComponentParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            open_doors_interaction: inst
                .get_str("openDoorsInteraction")
                .map(String::from)
                .unwrap_or_default(),
            close_doors_interaction: inst
                .get_str("closeDoorsInteraction")
                .map(String::from)
                .unwrap_or_default(),
        }
    }
...
impl Pooled for SecurityClearanceGiverComponentParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools
            .transitsystem
            .security_clearance_giver_component_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools
            .transitsystem
...
impl<'a> Extract<'a> for SecurityClearanceGiverComponentParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            security_tokens: inst
                .get_array("securityTokens")
                .map(|arr| {
                    arr.filter_map(|v| {
                ...
                .unwrap_or_default(),
            grant_when_inside_hosted_zone: inst
                .get_bool("grantWhenInsideHostedZone")
                .unwrap_or_default(),
            grant_when_inside_linked_area: inst
                .get_bool("grantWhenInsideLinkedArea")
                .unwrap_or_default(),
        }
    }
}
...
impl Pooled for SBindingTriggerGameplayTrigger {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.sbinding_trigger_gameplay_trigger
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.sbinding_trigger_gameplay_trigger
    }
}
...
impl Pooled for TransitNodeDialogueContext {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_node_dialogue_context
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_node_dialogue_context
    }
}
...
impl Pooled for TransitCarriageAudio {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_carriage_audio
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_carriage_audio
    }
}
...
impl<'a> Extract<'a> for TransitCarriageAudio {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            dialogue_contexts: inst
                .get_array("dialogueContexts")
                .map(|arr| {
                    arr.filter_map(|v| match v {
                        Value::Class { struct_index, data } => {
                            Some(b.alloc_nested::<TransitNodeDialogueContext>(
                                Instance::from_inline_data(b.db, struct_index, data),
                                false,
                            ))
                        }
                        Value::ClassRef(r) => Some(b.alloc_nested::<TransitNodeDialogueContext>(
                            b.db.instance(r.struct_index, r.instance_index),
                            true,
                        )),
                        _ => None,
...
impl Pooled for TransitCarriageEffects {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_carriage_effects
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_carriage_effects
    }
}
...
impl Pooled for TransitCarriageParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_carriage_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_carriage_params
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            default_interior_oc: match inst.get("defaultInteriorOC") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<GlobalResourceObjectContainer>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            audio: match inst.get("audio") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<TransitCarriageAudio>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            effects: match inst.get("effects") {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<TransitCarriageEffects>(
                        Instance::from_inline_data(b.db, struct_index, data),
                        false,
                    ))
                }
            ...
            fragment: inst
                ...
                .map(String::from)
                .unwrap_or_default(),
            fragment_arrived: inst
                .get_str("fragmentArrived")
                .map(String::from)
                .unwrap_or_default(),
            fragment_arriving: inst
                .get_str("fragmentArriving")
                .map(String::from)
                .unwrap_or_default(),
            fragment_departing: inst
                .get_str("fragmentDeparting")
                .map(String::from)
                .unwrap_or_default(),
            fragment_departed: inst
                .get_str("fragmentDeparted")
                .map(String::from)
                .unwrap_or_default(),
...
impl Pooled for TransitLimboNodeParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_limbo_node_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_limbo_node_params
    }
}
...
impl Pooled for TransitGatewayParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_gateway_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_gateway_params
    }
}
...
impl Pooled for TransitInteractionPanelParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_interaction_panel_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_interaction_panel_params
    }
}
...
impl Pooled for TransitDestinationParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_destination_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_destination_params
    }
}
...
impl<'a> Extract<'a> for TransitDestinationParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            ...
            radius: inst.get_f32("radius").unwrap_or_default(),
            unlocked_manufacturers: inst
                .get_array("unlockedManufacturers")
                .map(|arr| {
                    arr.filter_map(|v| {
                        if let Value::Reference(Some(r)) = v {
                            Some(r.guid)
...
impl Pooled for TransitManagerParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.transitsystem.transit_manager_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.transitsystem.transit_manager_params
    }
}
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            carriage_type: inst
                ...
                .and_then(|v| v.as_record_ref())
                .map(|r| r.guid),
            carriage_display_type_id: inst
                .get_str("carriageDisplayTypeId")
                .map(LocaleKey::from)
                .unwrap_or_default(),
            ...
            ideal_time_between_arrivals: inst
                .get_f32("idealTimeBetweenArrivals")
                .unwrap_or_default(),
            ...
            force_carriage_attach_to_parent: inst
                .get_bool("forceCarriageAttachToParent")
                .unwrap_or_default(),
            ...
            sort_destinations_by_priority: inst
                .get_bool("sortDestinationsByPriority")
                .unwrap_or_default(),
            ignore_door_proximity: inst.get_bool("ignoreDoorProximity").unwrap_or_default(),
            carriage_door_open_by_default: inst
                .get_bool("carriageDoorOpenByDefault")
                .unwrap_or_default(),
            ...
            reverse_display_index_order: inst
                .get_bool("reverseDisplayIndexOrder")
                .unwrap_or_default(),
            turn_on_interaction_name: inst
                .get_str("turnOnInteractionName")
                .map(String::from)
                .unwrap_or_default(),
            turn_off_interaction_name: inst
                .get_str("turnOffInteractionName")
                .map(String::from)
                .unwrap_or_default(),
            display_index_offset: inst.get_i32("displayIndexOffset").unwrap_or_default(),
            update_destinations_for_distant_trains: inst
                .get_bool("updateDestinationsForDistantTrains")
                .unwrap_or_default(),
            persist_destination_enabled_state: inst
                .get_bool("persistDestinationEnabledState")
                .unwrap_or_default(),
