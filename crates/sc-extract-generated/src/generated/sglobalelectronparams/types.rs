...
impl Pooled for SGlobalElectronParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.sglobalelectronparams.sglobal_electron_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.sglobalelectronparams.sglobal_electron_params
    }
}
...
impl<'a> Extract<'a> for SGlobalElectronParams {
    ...
    fn extract(inst: &Instance<'a>, b: &mut Builder<'a>) -> Self {
        Self {
            ...
            residual_charge_multiplier: inst
                .get_f32("residualChargeMultiplier")
                .unwrap_or_default(),
            residual_charge_interference: inst
                .get_f32("residualChargeInterference")
                .unwrap_or_default(),
            ...
            chain_lightning_effect_duration: inst
                .get_f32("chainLightningEffectDuration")
                .unwrap_or_default(),
            chain_lightning_joint: inst
                .get_str("chainLightningJoint")
                .map(String::from)
                .unwrap_or_default(),
            explosion_joint: inst
                .get_str("explosionJoint")
                .map(String::from)
                .unwrap_or_default(),
            ...
            chain_lightning_source_audio_start_trigger: match inst
                .get("chainLightningSourceAudioStartTrigger")
            {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<GlobalResourceAudio>(
                        Instance::from_inline_data(b.db, struct_index, data),
            ...
            chain_lightning_source_audio_stop_trigger: match inst
                .get("chainLightningSourceAudioStopTrigger")
            {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<GlobalResourceAudio>(
                        Instance::from_inline_data(b.db, struct_index, data),
            ...
            chain_lightning_target_audio_start_trigger: match inst
                .get("chainLightningTargetAudioStartTrigger")
            {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<GlobalResourceAudio>(
                        Instance::from_inline_data(b.db, struct_index, data),
            ...
            chain_lightning_target_audio_stop_trigger: match inst
                .get("chainLightningTargetAudioStopTrigger")
            {
                Some(Value::Class { struct_index, data }) => {
                    Some(b.alloc_nested::<GlobalResourceAudio>(
                        Instance::from_inline_data(b.db, struct_index, data),
            ...
            chain_lightning_particle_strength_rtpc: match inst
                .get("chainLightningParticleStrengthRtpc")
            {
                Some(Value::Class { struct_index, data }) => Some(b.alloc_nested::<AudioRtpc>(
                    Instance::from_inline_data(b.db, struct_index, data),
                    false,
            ...
            audio_bone_name: inst
                .get_str("audioBoneName")
                .map(String::from)
                .unwrap_or_default(),
        }
    }
