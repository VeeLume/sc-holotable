...
impl Pooled for RefiningProcess {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.refiningprocess.refining_process
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.refiningprocess.refining_process
    }
}
...
impl<'a> Extract<'a> for RefiningProcess {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            refining_speed: RefiningSpeed::from_dcb_str(
                inst.get_str("refiningSpeed").unwrap_or(""),
            ),
            refining_quality: RefiningQuality::from_dcb_str(
                inst.get_str("refiningQuality").unwrap_or(""),
            ),
            process_name: inst
                .get_str("processName")
                .map(LocaleKey::from)
