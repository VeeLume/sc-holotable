...
impl Pooled for JournalEntryAudioLog {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.journalentry.journal_entry_audio_log }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.journalentry.journal_entry_audio_log }
}
...
impl<'a> Extract<'a> for JournalEntryAudioLog {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            audio_log_name: inst.get_str("AudioLogName").map(String::from).unwrap_or_default(),
            description: inst.get_str("Description").map(LocaleKey::from).unwrap_or_default(),
            transcript: inst.get_str("Transcript").map(LocaleKey::from).unwrap_or_default(),
        }
    }
}
...
impl Pooled for JournalEntryDialogueLog {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.journalentry.journal_entry_dialogue_log }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.journalentry.journal_entry_dialogue_log }
}
...
impl<'a> Extract<'a> for JournalEntryDialogueLog {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            dialogue: inst.get_array("Dialogue")
...
impl Pooled for JournalEntryVideo {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.journalentry.journal_entry_video }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.journalentry.journal_entry_video }
}
...
impl<'a> Extract<'a> for JournalEntryVideo {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            video_def: inst.get("videoDef").and_then(|v| v.as_record_ref()).map(|r| r.guid),
            description: inst.get_str("description").map(LocaleKey::from).unwrap_or_default(),
        }
    }
}
...
impl<'a> Extract<'a> for SReputationStandingJournalEntryParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            change_direction: EReputationChangeReason::from_dcb_str(inst.get_str("changeDirection").unwrap_or("")),
        ...
        Self {
            standing_entries: inst.get_array("standingEntries")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<SReputationStandingJournalEntryParams>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
        ...
        Self {
            ...
            journal_groups: inst.get_array("journalGroups")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<SReputationJournalGroupParams>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
        ...
        Self {
            reputation_types: inst.get_array("reputationTypes")
                .map(|arr| arr.filter_map(|v| match v {
                        ...
                        Value::ClassRef(r) => Some(b.alloc_nested::<SReputationJournalEntriesParams>(b.db.instance(r.struct_index, r.instance_index), true)),
                        _ => None,
                    }).collect())
                .unwrap_or_default(),
        }
