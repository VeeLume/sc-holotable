...
impl Pooled for GlobalTutorialParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> {
        &pools.globaltutorialparams.global_tutorial_params
    }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> {
        &mut pools.globaltutorialparams.global_tutorial_params
    }
}
...
impl<'a> Extract<'a> for GlobalTutorialParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            valid_starting_areas: inst
                .get_array("validStartingAreas")
                .map(|arr| {
