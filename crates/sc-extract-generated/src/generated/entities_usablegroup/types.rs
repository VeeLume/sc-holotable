...
impl Pooled for SUsableGroupParams {
    fn pool(pools: &DataPools) -> &Vec<Option<Self>> { &pools.entities_usablegroup.susable_group_params }
    fn pool_mut(pools: &mut DataPools) -> &mut Vec<Option<Self>> { &mut pools.entities_usablegroup.susable_group_params }
}
...
impl<'a> Extract<'a> for SUsableGroupParams {
    ...
    fn extract(inst: &Instance<'a>, _b: &mut Builder<'a>) -> Self {
        Self {
            selection_method: EUsableSelectionMethod::from_dcb_str(inst.get_str("selectionMethod").unwrap_or("")),
