//! Dump the reputation → mission-reward bonus config from the raw DCB, to work
//! out the `[0/5/10/15/20]%` rep bonus evergr3n described: which record holds
//! the tiers, how it's gated ("factions that support it"), and how the tier is
//! chosen. Inspects SReputationMissionRewardBonusParams + a few faction /
//! standing records for the linkage.
//!
//! ```bash
//! cargo run -p sc-missions --release --example rep_bonus_dump
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (ty, cap) in [
        ("SReputationMissionRewardBonusParams", 8usize),
        ("FactionReputation", 3),
        ("SReputationStandingParams", 8),
        ("SReputationRewardAmount", 2),
    ] {
        let recs: Vec<&sc_extract::RecordPath> = paths
            .iter()
            .filter(|r| paths.type_name(r.struct_index) == Some(ty))
            .collect();
        println!("\n##### {ty}  ({} records) #####", recs.len());
        for rp in recs.iter().take(cap) {
            println!("--- {} ({}) ---", rp.name, rp.path);
            if let Some(rec) = db.record(&rp.guid) {
                dump(db, &rec.as_instance(), 1, 0, &paths);
            }
        }
    }
    Ok(())
        ...
        match prop.value {
            ...
            Value::Array(_) => {
                ...
                if let Some(arr) = inst.get_array(n) {
                    for (i, el) in arr.enumerate().take(40) {
                        match el {
                            Value::ClassRef(r)
