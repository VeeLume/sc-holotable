...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Name\tDisplay\tS\tSustain\tState\tResource\tAmount/s\tMinFraction\tPwrLow\tPwrMed\tPwrHigh");
    ...
    for (&guid, &handle) in ecd_map {
        ...
        if name.contains("CML") || name.contains("Mining") || name.contains("Salvage") { continue; }

        let has_weapon = ecd.components.iter().any(|c|
            matches!(c, DataForgeComponentParamsPtr::SCItemWeaponComponentParams(_)));
        ...
        let has_heat = wp.and_then(|w| w.connection_params.and_then(|h| h.get(pools)))
            .and_then(|c| c.simplified_heat_params).and_then(|h| h.get(pools)).is_some();
        let has_regen = wp.and_then(|w| w.weapon_regen_consumer_params.and_then(|h| h.get(pools))).is_some();
        ...
        let Some(irc) = irc else { continue; };

        for state_h in &irc.states {
            let Some(state) = state_h.get(pools) else { continue };

            // Power ranges
            let pwr_str = state.power_ranges.and_then(|h| h.get(pools)).map(|pr| {
                let low = pr.low.and_then(|h| h.get(pools)).map(|r| format!("{}:{:.2}", r.start, r.modifier)).unwrap_or_default();
                let med = pr.medium.and_then(|h| h.get(pools)).map(|r| format!("{}:{:.2}", r.start, r.modifier)).unwrap_or_default();
                let high = pr.high.and_then(|h| h.get(pools)).map(|r| format!("{}:{:.2}", r.start, r.modifier)).unwrap_or_default();
                (low, med, high)
            }).unwrap_or_default();
            ...
            for delta in &state.deltas {
                match delta {
                    ItemResourceDeltaBasePtr::ItemResourceDeltaConsumption(h) => {
                        ...
                        let Some(amount) = cons.consumption.and_then(|h| h.get(pools)) else { continue };

                        let resource = format!("{:?}", amount.resource);
                        let amount_val = amount.resource_amount_per_second.as_ref().map(|u| {
                            match u {
                                SBaseResourceUnitPtr::SStandardResourceUnit(h) =>
                                    h.get(pools).map(|v| v.standard_resource_units).unwrap_or(0.0),
                        ...
                        println!("{}\t{}\t{}\t{}\t{}\t{}\t{:.3}\t{:.2}\t{}\t{}\t{}",
                            name, display, size, sustain,
                            state.name, resource, amount_val,
                            cons.minimum_consumption_fraction,
                            pwr_str.0, pwr_str.1, pwr_str.2);
                    }
                    _ => {}
                }
            }
