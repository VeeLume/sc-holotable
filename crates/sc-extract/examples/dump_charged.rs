...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Name\tDisplay\tS\tType\tSustain\tChargeTime\tOverchargeTime\tOverchargedTime\tCooldown\tAutoCharge\tAutoFire\tFullOnly\tInterpolate\tInnerAction\tInnerRPM\tMaxChargeMod\tPhys\tEnergy\tDist\tSpeed\tLifetime\tActions");
    ...
    for (&guid, &handle) in ecd_map {
        ...
        for (i, action) in wp.fire_actions.iter().enumerate() {
            if matches!(action, SWeaponActionParamsPtr::SWeaponActionFireChargedParams(_)) {
                charged_idx = Some(i);
                break;
            }
        }
        ...
        // Inner action
        let inner_desc = charged.weapon_action.as_ref().map(|a| describe_action(a, pools)).unwrap_or("None".into());

        // Max charge modifier
        let charge_mod = charged.max_charge_modifier.and_then(|h| h.get(pools));
        let mod_desc = charge_mod.map(|m| {
            let mut parts = Vec::new();
            if m.damage_multiplier != 1.0 && m.damage_multiplier != 0.0 { parts.push(format!("dmg={:.2}x", m.damage_multiplier)); }
            if m.fire_rate != 0 { parts.push(format!("rpm={}", m.fire_rate)); }
            if m.projectile_speed_multiplier != 1.0 && m.projectile_speed_multiplier != 0.0 { parts.push(format!("speed={:.2}x", m.projectile_speed_multiplier)); }
            if m.pellets != 0 { parts.push(format!("pellets={}", m.pellets)); }
            if m.ammo_cost != 0 { parts.push(format!("ammoCost={}", m.ammo_cost)); }
            if m.ammo_cost_multiplier != 1.0 && m.ammo_cost_multiplier != 0.0 { parts.push(format!("ammoCostMult={:.2}", m.ammo_cost_multiplier)); }
            if m.heat_generation_multiplier != 1.0 && m.heat_generation_multiplier != 0.0 { parts.push(format!("heatMult={:.2}", m.heat_generation_multiplier)); }
            if m.charge_time_multiplier != 1.0 && m.charge_time_multiplier != 0.0 { parts.push(format!("chargeMult={:.2}", m.charge_time_multiplier)); }
            if parts.is_empty() { "default".into() } else { parts.join(" ") }
        }).unwrap_or("none".into());

        // Ammo
        let ammo = resolve_ammo(ecd, wp, pools, ecd_map, ammo_map);
        let (p, e, d, spd, lt) = ammo.map(|a| {
            let (dp, de, dd) = extract_damage(a, pools);
            (dp, de, dd, a.speed, a.lifetime)
        }).unwrap_or_default();

        println!("{}\t{}\t{}\t{}\t{}\t{:.2}\t{:.2}\t{:.2}\t{:.2}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{:.1}\t{:.1}\t{:.1}\t{:.0}\t{:.1}\t{}",
            name, display, size, item_type, sustain,
            charged.charge_time, charged.overcharge_time, charged.overcharged_time, charged.cooldown_time,
            charged.charge_automatically, charged.fire_automatically_on_full_charge,
            charged.fire_only_on_full_charge, charged.interpolate_charge_bonus,
            inner_desc, "", mod_desc,
            p, e, d, spd, lt,
            wp.fire_actions.len(),
        );
...
fn describe_action(action: &SWeaponActionParamsPtr, pools: &DataPools) -> String {
    match action {
        SWeaponActionParamsPtr::SWeaponActionFireRapidParams(h) =>
            h.get(pools).map(|r| format!("FireRapid(rpm={} hps={:.1})", r.fire_rate, r.heat_per_shot)).unwrap_or("FireRapid(?)".into()),
        SWeaponActionParamsPtr::SWeaponActionFireSingleParams(h) =>
            h.get(pools).map(|s| format!("FireSingle(rpm={} hps={:.1})", s.fire_rate, s.heat_per_shot)).unwrap_or("FireSingle(?)".into()),
...
fn resolve_ammo<'a>(ecd: &EntityClassDefinition, wp: &SCItemWeaponComponentParams, pools: &'a DataPools,
    ecd_map: &HashMap<Guid, Handle<EntityClassDefinition>>, ammo_map: &HashMap<Guid, Handle<AmmoParams>>) -> Option<&'a AmmoParams> {
    if let Some(guid) = wp.ammo_container_record {
        if let Some(&h) = ammo_map.get(&guid) { if let Some(a) = h.get(pools) { return Some(a); } }
        if let Some(&ch) = ecd_map.get(&guid) {
            if let Some(ce) = ch.get(pools) {
                let ac = ce.components.iter().find_map(|c| match c {
                    DataForgeComponentParamsPtr::SAmmoContainerComponentParams(h) => h.get(pools), _ => None,
                });
                if let Some(ac) = ac { if let Some(g2) = ac.ammo_params_record { if let Some(&h2) = ammo_map.get(&g2) { return h2.get(pools); } } }
            }
        }
    }
...
fn extract_damage(ammo: &AmmoParams, pools: &DataPools) -> (f32, f32, f32) {
    ...
    if let Some(ProjectileParamsPtr::BulletProjectileParams(h)) = &ammo.projectile_params {
        if let Some(b) = h.get(pools) {
            if let Some(DamageBasePtr::DamageInfo(dh)) = &b.damage { if let Some(di) = dh.get(pools) { p += di.damage_physical; e += di.damage_energy; d += di.damage_distortion; } }
            if let Some(det) = b.detonation_params.and_then(|h| h.get(pools)) {
                if let Some(expl) = det.explosion_params.and_then(|h| h.get(pools)) {
                    if let Some(DamageBasePtr::DamageInfo(dh)) = &expl.damage { if let Some(di) = dh.get(pools) { p += di.damage_physical; e += di.damage_energy; d += di.damage_distortion; } }
                }
            }
        }
    }
