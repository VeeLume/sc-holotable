...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (&guid, &handle) in ecd_map {
        ...
        let (dmg_phys, dmg_energy, dmg_dist) =
            ammo.map(|a| extract_damage(a, pools)).unwrap_or_default();
    ...
    for (name, display, _, info) in &entries {
        println!(
            "{} ({}) S{} {} {} actions={}",
            name, display, info.size, info.item_type, info.sustain, info.num_fire_actions
        );
        println!(
            "  mode: {} | alpha: phys={:.1} energy={:.1} dist={:.1}",
            info.mode, info.dmg_phys, info.dmg_energy, info.dmg_dist
        );
        for (i, e) in info.seq_entries.iter().enumerate() {
            println!(
                "  seq[{}]: delay={:.3} {} reps={} -> {}",
                i, e.delay, e.unit, e.repetitions, e.inner_action
            );
        }
        println!();
    }
    ...
    println!(
        "=== SUMMARY ({} unique base Sequence weapons) ===",
        entries.len()
    );
    println!("\nSequence modes:");
    for (k, v) in &mode_counts {
        println!("  {}: {}", k, v);
    }
...
fn describe_action(action: &SWeaponActionParamsPtr, pools: &DataPools) -> String {
    match action {
        SWeaponActionParamsPtr::SWeaponActionFireRapidParams(h) => h
            ...
            .map(|r| {
                format!(
                    "FireRapid rpm={} hps={:.2} spin={:.2}/{:.2}",
                    r.fire_rate, r.heat_per_shot, r.spin_up_time, r.spin_down_time
                )
            })
        ...
        SWeaponActionParamsPtr::SWeaponActionFireBurstParams(h) => h
            .get(pools)
            .map(|b| format!("FireBurst rpm={} shots={}", b.fire_rate, b.shot_count))
