//! Detailed dump of all FireRapid weapons with full ammo/damage resolution.
//!
//! ```bash
//! cargo run -p sc-extract --release --features entities-scitem-weapons,ammoparams \
//!     --example dump_fire_rapid
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (&guid, &handle) in ecd_map {
        ...
        let Some(rapid) = rapid else { continue };
        ...
        let (max_rounds_overheat, max_time_overheat) = if let Some(h) = heat {
            if rapid.heat_per_shot > 0.0 {
                let rounds = (h.overheat_temperature / rapid.heat_per_shot).floor();
                let time = rounds / (rapid.fire_rate as f32 / 60.0);
...
fn extract_ammo_data(
    ...
    let mut dist = 0.0f32;
    let mut pen_base = 0.0f32;
    let mut pen_near = 0.0f32;
    let mut pen_far = 0.0f32;
            ...
            ProjectileParamsPtr::BulletProjectileParams(h) => {
                if let Some(bullet) = h.get(pools) {
                    ...
                    if let Some(pen_h) = bullet.detonation_params.and_then(|h| h.get(pools)) {
                        if let Some(expl) = pen_h.explosion_params.and_then(|h| h.get(pools)) {
