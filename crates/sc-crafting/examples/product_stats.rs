...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let show = |label: &str, stats: &[sc_crafting::ProductStat]| {
        ...
        for ps in stats {
            ...
            let modified = ps
                ...
                .unwrap_or_else(|| "—".into());
            println!(
                "  {label:<22} {base:>10} {modified:>10} {:>+8.2}%",
                ps.pct_change()
            );
        }
    };
    ...
    for target in ["behr_sniper_ballistic_01", "klwe_pistol_energy_01"] {
        if let Some(guid) = find(target) {
            show(
                &format!("{target} @ Q{Q}"),
                &blueprints.product_stats(guid, &gp, &weapons, Q),
            );
        }
    }
    ...
    for target in ["rsi_deckcrew_armor_light_arms_01_01_01"] {
        if let Some(guid) = find(target) {
            show(
                &format!("{target} @ Q{Q}"),
                &blueprints.product_stats(guid, &gp, &armor, Q),
            );
        }
    }
    ...
    if let Some(guid) = find("cds_legacy_armor_heavy_core_01_01_01") {
        show(
            "ADP Core (cds_legacy_armor_heavy_core_01_01_01) @ Q0",
            &blueprints.product_stats(guid, &gp, &armor, 0),
        );
    }
    ...
    for target in [
        ...
        if let Some(guid) = find(target) {
            show(
                &format!("{target} @ Q{Q}"),
                &blueprints.product_stats(guid, &gp, &ship, Q),
            );
        }
    }
    ...
    if let Some(guid) = find("Mining_Laser_THCN_Impact_S1") {
        show(
            &format!("Impact Mining Laser @ Q{Q}"),
            &blueprints.product_stats(guid, &gp, &ship_wpn, Q),
        );
    }
