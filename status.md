...
**Resource-gathering data-source investigation (2026-06-10)** — no commit yet; doc-only. Mapped where ship/ROC/FPS mining, plants, and salvage/debris data lives in the DCB + object containers. **Headline: the per-location distribution IS in the data** (reversing an earlier wrong "telemetry-only" conclusion). One record per body — `HarvestableProviderPreset` (49) — is the generic world resource spawner; group/element probabilities reproduce every SCMDB number exactly. Signal = per-rock `signatures[4]` (Resource channel) ×1000; quality (B1–B8) is the existing `sc-crafting` system via `ResourceType.properties[ResourceTypeCraftingData]`. Verified against LIVE via a 60k-record XML corpus + expanded socpaks + a 12-agent verification workflow (all high-confidence). **Full spec: `docs/resource-gathering.md`.** Open: the body→provider join lives only in binary `.soc` (CrChF) files — needs a SOC parser or a name-heuristic stopgap; crate not yet scaffolded.
...
**sc-missions v2 redesign — all 7 phases shipped (2026-04-30 → 2026-05-01)** — `57115c9` … `9513c4d`. Mission-centric API replacing the v1 generator-shaped surface. See `docs/sc-missions-v2.md` for the full design.
...
| 5 | `b271e82` | `Contract → Mission`, `ContractIndex → Missions`, `MissionOrigin` consolidates handler fields |
...
**Investigation tooling (2026-05-01)** — committed as standalone examples in `crates/sc-missions/examples/`:
...
| Crate / Tool | Status |
|---|---|
| `sc-discovery` | Complete. 51 tests + 1 doctest. |
...
- **sc-langpatch migration to v2.** The langpatch feature-request docs (`docs/feature-request-sc-missions-langpatch.md`, `-sc-weapons-langpatch.md`) were written against v1; sections need re-anchoring on v2 type names.
...
3. `cargo check -p sc-missions --all-targets --features tui` to verify warm build.
4. Smoke test: `cargo run -p sc-explorer --release` (interactive TUI).
5. Quick analytics: `cargo run -p sc-missions --release --example encounter_analytics`.
