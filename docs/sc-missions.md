# `sc-missions` — v1 design specification (historical)

> ⚠️ **Superseded by [sc-missions-v2.md](sc-missions-v2.md).** This file documents the v1 design that shipped 2026-04-24 and was replaced by v2 over 2026-04-30 → 2026-05-01. Keys concepts that no longer exist in the implementation: `ContractIndex` (now `Missions`), the implicit merge step (now explicit pools), `Variation` (now sibling Missions), `find_bp_conflicts` / `cluster_by_*` (replaced by `MissionPools` + divergence helpers), `EncounterGroup` / `EncounterWave` / `SpawnContext` (now `Encounter` enum + `EncounterPhase<S>` + `TagBag`).
>
> The crate's current consumer-facing API is documented in [sc-missions-guide.md](sc-missions-guide.md). This file is preserved as the design history that motivated v2.

## Purpose

`sc-missions` provides the domain layer for Star Citizen's contract / mission system. It answers the question **"what contracts are available in the game, and what does each one do?"** by walking the DCB's static generator / handler / contract graph, expanding each generator into the concrete variations it can produce, resolving every reference the contract touches (rewards, prerequisites, spawns, locations), and merging near-duplicate variations into a single effective contract.
...
| `streamdeck-starcitizen` | Does **not** use `sc-missions`. |

## Scope

**What `sc-missions` owns:**
...
- **(Dropped — use `sc_extract::Tags`.)** The spec originally proposed a `TagRegistry` in this crate. `sc_extract::Tags` (already built into every `DatacoreSnapshot`) provides everything it would have — GUID ↔ name maps, hierarchical `ancestors` / `descendants` / `is_descendant_of` walks, root-to-leaf path display. Reach through `datacore.snapshot().tag_tree` instead of adding a redundant registry. Dropping this also removes a step from §"Implementation sequencing".
- **Ship-entity registry** (`Ships`). Indexes every `EntityClassDefinition` that is **reachable from a contract spawn query** by its tag set, display name, and size. Enables `resolve(positive_tags, negative_tags) → Vec<ShipCandidate>` for spawn descriptions. Built once from the DCB, data-derived (see §"Ship-entity registry" — no name-pattern filtering).
...
**What `sc-missions` does not own:**

- Generated raw types — they live in `sc_extract::generated`. Escape hatch for anything the model doesn't cover is `Datacore::db()` → svarog; consumers reach through that directly rather than a sc-missions raw layer.
- ~~Localization string resolution~~ (**scope change 2026-04-24**). Originally deferred to consumers, but contract title / description resolution in particular required a non-trivial inheritance walk (contract → sub-contract → handler → template) that every consumer would otherwise reinvent — and `paramOverrides`-only walkers silently return empty strings for entire contract families (TarPits, Adagio, …). `sc_missions::resolve_contract_text` now owns this; [`Contract.title`] and [`Contract.description`] are resolved `String`s. Consumers still use `sc_extract::LocaleMap` for general localisation needs; sc-missions just absorbs the contract-specific resolution logic.
...
These caches are currently built ad-hoc inside sc-langpatch's `mission_enhancer.rs`. Putting them in sc-missions — the first consumer that genuinely needs them — keeps the workspace lean. When a second crate needs them (sc-entities, sc-loadouts), they graduate into a shared helper. Today that's premature.
...
│              Ships (entities reachable from spawn    │
│                            queries in the contract graph)   │
│              BlueprintPools  (pools → item sets)      │
│              RewardCurrencies  (aUEC / scrip GUIDs)     │
└──────────────────────────────────────────────────────────────┘
            │
            ▼
┌──────────────────────────────────────────────────────────────┐
...
pub struct ContractIndex {
    ...
    // tags reached via sc_extract::Tags (see Scope §)
}
...
pub struct Contract {
    ...
    // Rewards (sum across reward list; currency disambiguated via RewardCurrencies)
    pub reward_uec:   i32,
...
|---|---|---|
| `ContractResult_Item.entityClass` | `RewardCurrencies` | `ScripReward` (faction-scoped), `ItemReward`, or plain UEC |
| `BlueprintRewards.blueprintPool` | `BlueprintPools` | `Vec<BlueprintItemRef { entity_class_guid, display_name: LocaleKey }>` |
...
| `MissionPropertyValue_ShipSpawnDescriptions` → `ships[].options[].tags` | `TagRegistry` + `Ships` | `Vec<ShipCandidate { display_name, size, match_tags }>` |
...
## Tag resolution: use `sc_extract::Tags`

sc-extract already builds a `Tags` from every DCB `Tag` record as part of `Datacore::parse` (under `DatacoreConfig::build_tag_tree`). It has GUID ↔ name maps, hierarchical navigation, and `path(&Guid) → Vec<&str>` for display. sc-missions does not duplicate this — reach through `datacore.snapshot().tag_tree` for any name resolution.

The small amount of sc-missions-specific tag classification (difficulty tags, ship-class tags, AI-skill parsing) lives as free functions in the crate's internal modules, not as methods on a new type. Each is a tiny string-set check or regex.

**Reference graph (`sc_extract::ReferenceGraph`).** Also available from the snapshot. Useful for reverse lookups — "which contracts reference this location template?" or "which contracts have this faction as a rep prerequisite?" — which we'll want for variation-merging diagnostics and future chain-graph reconstruction (v2). Not used by the ship registry or expansion logic in v1, but consumers (and the census) can reach for it without sc-missions re-implementing it.
...
**Query semantics — three tag classes via the tag tree.** The `sc_extract::Tags` has 65 top-level roots (`Ship`, `AI`, `Missions`, `UI`, …). Classification is purely tree-based (workspace rule §5):
...
The tag tree itself has only `children` references in the DCB — no `parent` field. `sc_extract::Tags::from_database` derives parents from children in a second pass so `ancestors()` / `path()` / `is_descendant_of()` work (fixed 2026-04-24).
...
Classification walks each tag's `Tags::path()` and dispatches on the root + first category. Works because the DCB organises spawn-context tags into principled subtrees (`AI ▸ Faction` / `AI ▸ CargoManifest` / `AI ▸ SkillDefinitions`) — the classifier respects that structure rather than pattern-matching names.
...
| Reward UEC | `ContractResult_CalculatedReward` (engine-computed at runtime — see below) for most contracts; occasional `ContractResult_Item` with aUEC entity_class for fixed amounts | `reward_uec: RewardAmount::{Calculated(CalcInputs), Fixed(i32), None}` — majority of contracts report `Calculated` because the DCB does not store the amount. `CalcInputs` carries the DCB scalars a community formula consumes (difficulty profile weights, difficulty enum levels, time_to_complete, buy_in). Estimator ships as opt-in in `sc_missions::estimators::estimate_uec`. |
| Reward Blueprint | `BlueprintRewards { chance, blueprintPool }` resolved through [`BlueprintPools`]. Two-stage item-name resolution: crafted-entity's `DisplayNameCache` entry (primary) → `CraftingBlueprint.blueprintName` locale lookup (fallback). CIG ships placeholder text `<= PLACEHOLDER =>` for un-finalised strings — detected and treated as unresolved. | `reward_blueprints: Option<BlueprintReward { chance, items: Vec<BlueprintItem { crafted_entity_guid, display_name, weight }> }>` |
| Reward Scripts | `ContractResult_Item` where `entityClass` is in `RewardCurrencies::scrip_kinds` | `reward_scrip: Vec<ScripReward { kind, amount }>` |
...
| Possible Ship Encounters | `paramOverrides.propertyOverrides[{ missionVariableName: *ShipSpawnDescriptions* \| *MissionTargets* \| *WaveShips*, value: MissionPropertyValue_ShipSpawnDescriptions }]` → `Ships::resolve_spawn` per option | `encounters: Vec<EncounterGroup { role, name, waves: Vec<Wave { count, ai_skill, candidates: Vec<ShipCandidate> }> }>` |
...
- `ContractResult_Item.entity_class` top-N — for `RewardCurrencies` calibration.
...
2. **Scrip disambiguation.** Which `entityClass` GUIDs are scrip currency vs generic items? Current hypothesis: a small fixed set, distinct per faction (Pyro scrip vs Stanton scrip). `contract_census`'s entity-class top-N will confirm. `RewardCurrencies` starts with a hardcoded list, derived from census output, kept in a `currency.rs` table.
...
2. **Registries.** ✅ All landed 2026-04-24.
   - `TagRegistry` **dropped** (use `sc_extract::Tags`).
   - `Ships` — 3,218 entities, 94.1% spawn-query coverage, intent-based resolver.
   - `BlueprintPools` — 45 pools, ~100% item-name resolution.
   ...
   - `RewardCurrencies` — typed via `SItemDefinition.type == EItemType::Currency`, 2 entities in 4.7 LIVE (merc scrip 99 uses, council scrip 84 uses). Banu favours intentionally classified as items (not `EItemType::Currency`).
3. **Expansion.** ✅ v2 landed 2026-04-24. [`expand_all`] produces 4,590 `ExpandedContract`s matching the census count. Each row carries the full contract picture after inheritance resolution:
   ...
   - **Encounters**: `Vec<EncounterGroup { variable_name, waves: Vec<EncounterWave { name, slots: Vec<EncounterSlot { concurrent, weight, candidates, context }> }> }>` — ship spawn queries resolved through `Ships::resolve_spawn` (intent-based with Ship-subtree classifier) with per-slot `SpawnContext` breakdown (AI skill, faction, cargo, arrival, …).
...
pub struct Locations { /* guid → LocationRef */ }
pub struct Localities { /* guid → LocalityView */ }

pub struct ExpandedContract { /* …, */ pub mission_span: Vec<Guid> }
pub struct Contract        { /* …, */ pub mission_span: Vec<Guid> }
```

**Implementation.** `Locations::build(&datacore)` walks every `StarMapObject` and iteratively resolves the `parent` chain (cycle-guarded). The root-most ancestor's record name is normalised (`PyroStar` → `Pyro`, `NyxSolarSystem` → `Nyx` — stripping `Star` / `SolarSystem` suffixes) to produce `SystemKey`. Body is `Some(name)` only when the chain has at least one intermediate node between the location and root, so Lagrange points / moons surface `body: Some("Pyro3")` without redundantly labelling top-level planets. `Localities::build(&datacore, &locations)` then resolves every `MissionLocality.available_locations` through the location registry, aggregating `BTreeSet<SystemKey>` per locality.

`expand_all` takes `&Localities` and populates `ExpandedContract.mission_span` from the Locality prereq chain (handler + contract + sub-contract), first-seen-wins-dedup. `merge_expansions` takes the union across variations for `Contract.mission_span`.

**Scope decisions.**
- `mission_span` is `Vec<Guid>`, not inline `Vec<LocalityView>`, to avoid duplicating ~100k LocationRefs across ExpandedContracts. Consumers look up through `Localities::get`.
