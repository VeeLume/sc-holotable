...
## Object-container binding — superseded

> Status: **superseded (2026-07).** The `LocationContainers` spec that lived here
> (system-OC scan of `OrbitingObjectContainer` → `SNavPointObjectMetadataParams`
> pairs) was implemented, then replaced: SC 4.8 renamed the metadata component to
> `SObjectMetadataParams` (the old scan found 0 bindings), and consumers needed
> the *nested* placements (sub-location OCs inside bodies), positions, and
> mission-CRC resolution the flat 1:1 map couldn't carry.
>
> The current surface is **`ObjectContainers`** (one tolerant socpak
> placement-graph walk, `ObjectContainers::cook`) joined with the DCB StarMap
> into **`Universe`** / **`Place`** — see `docs/object-containers.md` and the
> `CHANGELOG.md` entry for the API. `sc-gathering` consumes `&ObjectContainers`.
