...
These refinements make the generated code more type-safe than a 1:1 schema reflection:
...
5. `git add -A` + one commit: `chore(datacore): regenerate bindings (SC <version>)` (Conventional Commits — the repo's `commit-msg` hook rejects other formats). Bails out cleanly if the generator produced an identical tree.
...
- **`RecordRef` doesn't implement serde at all.** Map DCB `Reference` fields to `Option<CigGuid>` and extract the inner guid, not `Option<RecordRef>`. See the generator's `rust_type_for` function for the mapping. To turn one of these GUIDs back into a typed value, use `Datacore::resolve::<T>(&guid)` (see "Typed value surfaces") rather than `db().record(guid)` + raw field access.
...
- **rustc stack bump was a serde-derive workaround, now removed.** Pre-derive-drop, `rustc` could overflow its 8 MB thread stack while parsing/typechecking the generated crate due to serde-derive proc-macro expansion across ~6.5k types. `.cargo/config.toml` used to set `RUST_MIN_STACK=67108864`. With serde derives gone, the default stack is sufficient — no compile-time env var in `.cargo/config.toml` anymore. If you hit `STATUS_STACK_OVERFLOW` inside *rustc itself* again, that's a regression: investigate what re-introduced the derive load rather than re-adding the env var.
- **Compiled-binary stack bump is STILL needed on Windows.** `.cargo/config.toml` sets `link-args=/STACK:8388608` for the Windows target. Windows defaults main-thread stack to 1 MB where Linux/macOS default to 8 MB. Under `--features full`, `parse_real_p4k` overflows the 1 MB default during DCB extract dispatch (deeply-nested generic helper chains, not infinite recursion). Don't confuse this with the old serde-derive story — it's a pure "Windows has a smaller default stack than other platforms" issue.
